...
import { useTicketsStore, type Status, statuses } from '@/stores/tickets';
...
const { updateTicketStatus, archiveTicket } = ticketStore;
...
<template>
    <div class="p-4 sm:p-6">
        <div class="flex items-center justify-between mb-4">
            ...
            <button v-if="filter === 'active'" @click="isModalOpen = true"
                class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md shadow-sm text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                Add New Ticket
            </button>
        </div>

        <div v-if="filteredTickets.length > 0" class="shadow overflow-x-auto border-b border-gray-200 sm:rounded-lg">
                ...
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th scope="col"
                            class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            Status</th>
                        <th scope="col" v-if="filter === 'active'"
                            class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                            Actions
                        </th>
                    </tr>
                ...
                <tbody class="bg-white divide-y divide-gray-200">
                    <tr v-for="ticket in filteredTickets" :key="ticket.id" class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            ...
                            <select
                                @change="updateTicketStatus(ticket.id, ($event.target as HTMLSelectElement).value as Status)"
                                class="rounded-md border-gray-300 shadow-sm focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50">
                                <option v-for="status in statuses" :key="status" :value="status"
                                    :selected="status === ticket.status">{{ status }}</option>
                            </select>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium" v-if="filter === 'active'">
                            <button v-if="ticket.status === 'Closed' && !ticket.isArchived"
                                @click="archiveAndNotify(ticket.id)" class="text-indigo-600 hover:text-indigo-900">
                                Archive
                            </button>
                        </td>
                    </tr>
