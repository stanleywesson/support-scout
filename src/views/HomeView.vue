...
<template>
  <div class="p-4 sm:p-6">
    ...
    <div v-else-if="filteredTickets.length > 0" class="shadow overflow-x-auto border-b border-gray-200 sm:rounded-lg">
          ...
          <tr v-for="ticket in filteredTickets" :key="ticket.id" class="hover:bg-gray-50">
            ...
            <td class="px-6 py-4 whitespace-nowrap">
              <select @change="
                ...
                <option v-for="status in statuses" :key="status" :value="status" :selected="status === ticket.status">
                  {{ status }}
                </option>
              </select>
            </td>
            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium" v-if="filter === 'active'">
              <button v-if="ticket.status === 'Closed' && !ticket.isArchived" @click="archiveAndNotify(ticket.id)"
