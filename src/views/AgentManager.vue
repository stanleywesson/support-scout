...
const { tickets, supportAgents, loading } = storeToRefs(ticketsStore)
const newAgentName = ref('')

onMounted(() => {
  if (supportAgents.value.length === 0) {
    ticketsStore.fetchAgents()
  }
...
const addAgent = () => {
  if (newAgentName.value.trim()) {
    ticketsStore.addAgent(newAgentName.value.trim())
    newAgentName.value = ''
  }
}

const removeAgent = (name: string) => {
  ticketsStore.removeAgent(name)
}
...
<template>
  <div class="p-4 sm:p-6">
    ...
    <div v-if="loading" class="text-center p-8">
      <h2 class="text-2xl font-bold text-gray-800">Loading agents...</h2>
    </div>

    <div v-else class="shadow overflow-x-auto border-b border-gray-200 sm:rounded-lg">
        ...
        <tbody class="bg-white divide-y divide-gray-200">
          <tr v-for="agent in agentsWithTicketCount" :key="agent.agent">
            ...
            <td class="px-6 py-4 whitespace-nowrap text-sm font-medium">
              <button
                @click="removeAgent(agent.agent)"
                :disabled="agent.count > 0"
                class="text-red-600 hover:text-red-900 disabled:text-gray-400 disabled"
              >
                Remove
              </button>
            </td>
